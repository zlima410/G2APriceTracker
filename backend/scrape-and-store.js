...
// Steam's storefront API isn't officially rate-limit documented, but it
// gets noticeably stricter once you're making a couple hundred requests
// in a single run. Bumped up from 500ms now that the catalog has grown —
// override via env var if you need to tune this without editing code.
const SCRAPE_DELAY_MS = Number(process.env.SCRAPE_DELAY_MS) || 1500;

async function scrapeGame(appid) {
  const response = await getWithRetry(axios, STEAM_API_BASEURL, {
...
async function main() {
  ...
  console.log(`Scraping ${games.length} games (delay: ${SCRAPE_DELAY_MS}ms)...`);
